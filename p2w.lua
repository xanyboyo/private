        ...
        local success, err = pcall(function()
            ...
            if not currentlyTweening and character then
                ...
                if closest and closest:FindFirstChild("CoinVisual") and distance > 0 then
                    ...
                    -- Wait for tween to complete, then start jittering with tweens
                    task.spawn(function()
                        ...
                        while coinValidationActive and currentTargetCoin do
                            ...
                            if not coinVisual then break end
                            ...
                            if hrp and character then
                                ...
                                -- Create quick jitter tween
                                local jitterTween = game:GetService("TweenService"):Create(
                                    hrp,
                                    TweenInfo.new(0.05, Enum.EasingStyle.Linear),
                                    {CFrame = jitterTargetCFrame}
                                )
                                jitterTween:Play()
                                jitterTween.Completed:Wait()
                            end
                        end
                    end)
                    ...
                    -- Simple positioning above the coin (no underground, no jitter)
                    local targetPosition = closest.Position + Vector3.new(0, 1.25, 0)
                    local rotation = CFrame.Angles(math.rad(90), 0, 0)
